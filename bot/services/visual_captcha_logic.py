...
async def is_visual_captcha_enabled(session: AsyncSession, chat_id: int) -> bool:
    """Проверяет включена ли визуальная капча для группы"""
    try:
        ...
        logger.info(f"visual_capcha_logic: Проверка визуальной капчи для группы {chat_id}: {'включена' if is_enabled else 'выключена'}")
